...
class WizardCloseBox(models.TransientModel):
    _name = 'wizard.close.box'
    _description = "Wizard de cierre de caja"
    ...
    def print_report_close_box_xls(self):
        self.ensure_one()
        [data] = self.read()
        datas = {
            'ids': self.id,
        ...
        return self.env.ref('advanced_reports.action_close_box_xls').report_action(self, data=datas)

    def print_report_close_box_pdf(self):
        self.ensure_one()
        [data] = self.read()
        datas = {
            'ids': self.id,
        ...
        return self.env.ref('advanced_reports.action_close_box_pdf').report_action(self, data=datas)