...
class WizardPlanillaF07(models.TransientModel):
    _name = 'wizard.planilla.f14'
    _description = "Wizard de reporte planilla F-14"
    ...
    def print_report_planilla_f14_xls(self):
        self.ensure_one()
        [data] = self.read()
        datas = {
            'ids': self.id,
            'model': 'wizard.planilla.f14',
            'form': data,
        }
        return self.env.ref('advanced_reports.action_report_planilla_f14_xls').report_action(self, data=datas)