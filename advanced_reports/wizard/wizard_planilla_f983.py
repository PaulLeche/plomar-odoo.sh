...
class WizardPlanillaF983(models.TransientModel):
    _name = 'wizard.planilla.f983'
    _description = "Wizard de reporte planilla F-983"
    ...
    def print_report_planilla_f983_xls(self):
        self.ensure_one()
        [data] = self.read()
        datas = {
            'ids': self.id,
            'model': 'wizard.planilla.f983',
            'form': data,
        }
        return self.env.ref('advanced_reports.action_report_planilla_f983_xls').report_action(self, data=datas)