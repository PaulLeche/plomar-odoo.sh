...
class WizardPlanillaF987(models.TransientModel):
    _name = 'wizard.planilla.f987'
    _description = "Wizard de reporte planilla F-987"
    ...
    def print_report_planilla_f987_xls(self):
        self.ensure_one()
        [data] = self.read()
        datas = {
            'ids': self.id,
            'model': 'wizard.planilla.f987',
            'form': data,
        }
        return self.env.ref('advanced_reports.action_report_planilla_f987_xls').report_action(self, data=datas)