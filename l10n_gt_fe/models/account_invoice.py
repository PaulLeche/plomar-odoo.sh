...
class AccountMove(models.Model):
    ...
    sent_arch_xml = fields.Text(string="Estructura XML", help='XML Architecture ', copy=False, readonly=True)
    ...
    fe_type = fields.Selection(TYPE_FE, string='Tipo', store=True, related='journal_id.fe_type', readonly=False)
    ...
    fe_serie = fields.Char(string='Serie', copy=False)
    fe_number = fields.Char(string='Number', copy=False)
    ...
    def _xml(self):
        ...
        ET.SubElement(Adenda, 'Vendedor').text = self.invoice_user_id.name if self.invoice_user_id else self.user_id.name
        ...
        if self.fe_type in ['RDON']:
            DatosGenerales = {
                ...
                'Tipo': self.fe_type,
                'TipoPersoneria': self.tipo_personeria
            }
        else:
            DatosGenerales = {
                'CodigoMoneda': self.currency_id.name,
        ...
        if self.company_id.afiliacion_iva:
            Emisor.set('AfiliacionIVA', self.company_id.afiliacion_iva)
        else:
        ...
        ET.SubElement(DireccionEmisor, 'dte:Direccion').text = '%s %s' % (self.company_id.street, self.company_id.street2 or '')
        ...
        if self.fe_type == 'FAEX':
            datosEmisor = {
                'CorreoReceptor': self.partner_id.email or self.company_id.fe_other_email or '',
        ...
        if self.fe_phrase_ids:
            ...
            for phrase in self.fe_phrase_ids:
                if phrase.name == 'Frases de Retención del ISR' and phrase.fecha_resolucion and phrase.numero_resolucion:
                    ET.SubElement(Frases, 'dte:Frase', {
                        'CodigoEscenario': phrase.code, 
                        'TipoFrase': phrase.type,
                        'FechaResolucion': phrase.fecha_resolucion.strftime("%Y-%m-%d"),
                        'NumeroResolucion': phrase.numero_resolucion
                    })
                else:
                    ET.SubElement(Frases, 'dte:Frase', {
                        'CodigoEscenario': phrase.code, 
        ...
        if self.fe_type in ('FESP', 'NDEB', 'NCRE', 'FCAM', 'FAEX'):
            ...
            if self.fe_type in ('NDEB', 'NCRE'):
                ...
                if (
                    not self.reversed_entry_id.fe_uuid or 
                    not self.reversed_entry_id.fe_number or 
                    not self.reversed_entry_id.fe_serie
                   ):
                    raise UserError("Error. No hay documento relacionado")
                ET.SubElement(Complemento, "cno:ReferenciasNota", {
                    ...
                    "MotivoAjuste": self.ref or '%s: %s' % (self.reversed_entry_id.name, self.reversed_entry_id.fe_uuid),
                    "NumeroAutorizacionDocumentoOrigen": self.reversed_entry_id.fe_uuid,
                    "NumeroDocumentoOrigen": self.reversed_entry_id.fe_number,
                    "SerieDocumentoOrigen": self.reversed_entry_id.fe_serie,
                    "Version": "0.0",
                    "xsi:schemaLocation": "http://www.sat.gob.gt/face2/ComplementoReferenciaNota/0.1.0 C:\\Users\\User\\Desktop\\FEL\\Esquemas\\GT_Complemento_Referencia_Nota-0.1.0.xsd"
                })

            elif self.fe_type == 'FESP':
                RetencionISR = RetencionIVA = 0.00
                Complemento = ET.SubElement(Complementos, 'dte:Complemento', {
                    "IDComplemento": "1",
                ...
                ET.SubElement(RetencionesFacturaEspecial, "cfe:TotalMenosRetenciones").text = str(round(self.amount_total - withhold_amount, 2))
    ...
    def send_invoice(self):
        if self.fe_active and self.company_id.fel_enabled:
            ...
            headers['identificador'] = self.name + "//" + str(self.id)
            payloads = {
    ...
    def cancel_dte(self):
        ...
        ET.SubElement(AnulacionDTE, 'dte:DatosGenerales', {
            'FechaEmisionDocumentoAnular': datetime(invd.year, invd.month, invd.day, 0, 0, 1).isoformat(), #"2020-03-04T00:00:00-06:00"
