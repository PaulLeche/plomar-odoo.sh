...
class AddressDepartamento(models.Model):
    _name = 'address.depart.fel'
    _description = 'address depart fel'
    _rec_name = 'name_depart'

    code = fields.Char()
    name_depart = fields.Char()
    partner_ids = fields.One2many('res.partner', 'fe_address_dep', string='Departamento')


class AddressMunicipio(models.Model):
    _name = 'address.muni.fel'
    ...
    partner_ids = fields.One2many('res.partner', 'fe_address_mun', string='Departamento')


class ExportCountry(models.Model):
    _name = 'export.country'
    ...
    partner_ids = fields.One2many('res.partner', 'fe_country', string='Departamento')
