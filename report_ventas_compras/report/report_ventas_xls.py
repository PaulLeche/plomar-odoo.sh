...
class ReportSaleBookXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, docids, data):
        ...
        sheet.merge_range('G10:G11', 'No. Documento', bold)
        ...
        for linea in lineas: 
            for line in linea[1]:
                ...
                if line['cliente'] == '***FACTURA ANULADA***':
                    sheet.write(row+2, column+8, '', no_bold_number)
                    sheet.write(row+2, column+9, '', no_bold_number)
                    sheet.write(row+2, column+10, '', no_bold_number)
                    sheet.write(row+2, column+11, '', no_bold_number)
                    sheet.write(row+2, column+12, '', no_bold_number)
                    sheet.write(row+2, column+13, '', no_bold_number)
                else:
                    sheet.write(row+2, column+8, float(line['bienes_gravados']), currency_number)
                    sheet.write(row+2, column+9, float(line['servicios_gravados']), currency_number)
                    sheet.write(row+2, column+10, float(line['bienes_exentos']), currency_number)
                    sheet.write(row+2, column+11, float(line['servicios_exentos']), currency_number)
                    sheet.write(row+2, column+12, float(line['iva']), currency_number)
                    sheet.write(row+2, column+13, float(line['subtotal']), currency_number)
        ...
        sheet.write(row+2, column+8, float(ultima['total_bienes_gravados']), currency_total)
        sheet.write(row+2, column+9, float(ultima['total_servicios_gravados']), currency_total)
        sheet.write(row+2, column+10, float(ultima['total_bienes_exentos']), currency_total)
        sheet.write(row+2, column+11, float(ultima['total_servicios_exentos']), currency_total)
        sheet.write(row+2, column+12, float(ultima['total_iva_final']), currency_total)
        sheet.write(row+2, column+13, float(ultima['total_subtotal_final']), currency_total)
        ...
        sheet.write('E'+str(row+8), float(ultima['total_bienes_exentos']), currency_total_amount)
        sheet.write('E'+str(row+9), float(ultima['total_servicios_gravados']), currency_total_amount)
        sheet.write('E'+str(row+10), float(ultima['total_servicios_exentos']), currency_total_amount)
        sheet.write('E'+str(row+11), float(ultima['total_iva_final']), currency_total_amount)
        sheet.write('E'+str(row+12), float(ultima['total_subtotal_final']), currency_total_amount)
        ...
        for total_fac in ultima['company_fac_total']: 
            ...
            sheet.write('L'+str(row+8), float(ultima['company_fac_total'].get(total_fac)[1]), currency_total_amount)
        ...
        sheet.write('L'+str(row+8), float(ultima['total_subtotal_final'],), currency_total_amount)